...
- SFT（Policy）：trace→数据集→训练→回放对比（见第 13 章；V1 必做）
...
为什么需要：
- 你的系统要支持：Trace 回放、Frozen Evidence Replay、评测回归、以及 **SFT 前后对比**；没有版本化，任何“对比结果”都可能是漂移造成的假差异。
...
- `policy_version`：`base` | `policy`（V1 必须支持 base/policy 对比；policy 由第 13 章训练产生）
...
### 1.2 系统模块拆分（V1 推荐，主模块）
拆分目标：
- **职责清晰**：导入、检索、编排、生成、观测、评估、SFT 各自独立。
- **依赖单向**：上层依赖下层；存储/LLM 作为可替换适配器。
- **可回放闭环**：Trace/Eval/SFT 都是系统一级能力（便于面试展示）。

V1 主模块（仅列核心闭环模块）：
1) **Ingestion**：Loader/Parser/Chunker/Locator；Raw/Clean 双流；索引写入；safe_summary 任务入队
2) **Retrieval**：structured filter、BM25(FTS5)、Dense(Chroma)、RRF 融合、可选 cross-encoder rerank、Evidence Pack 组装
3) **Orchestrator**：Hard Guard、PROBE、plan_check、LLM replan、Clarification Loop（核心槽位≤2）
4) **Agents**
   - BJJ Coach：EvidenceSummary + Gate（三态）+ 生成 + Validator + repair/degrade
   - Literary：NOTES 域 hybrid 检索 + anchors prompt（top-1 excerpt + top-2/3 summary）+ 创作性生成
5) **Observability**：trace/span/event 打点、runtime_config_snapshot、成本/延迟统计
6) **Evaluation**：Golden set 管理、Frozen replay、RAGAS、自定义硬指标、LLM-as-judge、人工 rubric
7) **SFT（Policy）**：trace→数据集→人工修订→训练→回放对比→回归（V1 即实现）

支撑层（不作为“主模块”单列，但必须存在）：
- Web UI / Backend API（承载交互与契约）
- Storage/Jobs（SQLite/FTS5/Chroma/FileStore + safe_summary/reindex/reembed）

### 1.3 项目目录结构（V1 建议）
目标：把“代码”和“可变数据（SQLite/Chroma/原文快照/trace）”分离，避免 demo 时污染仓库；同时让模块边界在目录上可见。
...
├─ server/                          # FastAPI 后端
│  ├─ app/
│  │  ├─ api/                       # 路由层（/chat/turn、/record、/traces、/eval、/profile…）
│  │  ├─ ingestion/                 # 主模块：导入流水线
│  │  ├─ retrieval/                 # 主模块：Hybrid 检索 + Evidence Pack
│  │  ├─ orchestrator/              # 主模块：对话编排（Probe/Replan/Clarify）
│  │  ├─ agents/
│  │  │  ├─ bjj_coach/              # Gate/Generator/Validator/Repair
│  │  │  └─ literary/               # anchors prompt + 创作性生成
│  │  ├─ observability/             # 主模块：trace/span/event + 指标聚合
│  │  ├─ evaluation/                # 主模块：RAGAS/Judge/回放评测
│  │  ├─ sft/                       # 主模块：数据集/训练/推理切换（policy）
│  │  ├─ core/                      # 共享：schema/枚举/错误码/版本化/runtime_config
│  │  ├─ storage/                   # 适配器：sqlite/fts5/chroma/filestore/trace_store
│  │  └─ jobs/                      # worker：safe_summary、reindex、reembed
│  └─ tests/
├─ web/                             # Next.js 前端
│  ├─ app/                          # Home/Chat/Traces/Evaluation 页面
│  ├─ components/                   # EvidencePanel/RecordForm/TraceViewer…
│  └─ lib/                          # API client、SSE、类型定义（与 server core 对齐）
├─ datasets/                        # 离线数据（纳入版本控制）
│  ├─ golden/                       # golden set（queries + 标注/期望）
│  └─ sft/                          # SFT 数据集版本（manifest + train.jsonl）
├─ data/                            # 运行时数据（不入库，便于清理/备份）
│  ├─ sqlite/                       # app.db
│  ├─ chroma/                       # chroma 持久化目录（按 embedding_version 隔离也可）
│  ├─ filestore/                    # doc_version 原文快照
│  ├─ traces/                       # 可选：裁剪快照/导出文件
│  └─ jobs/                         # worker 中间产物（可选）
├─ scripts/                         # 维护脚本（导入/重建/导出/本地跑 eval）
└─ DEV_SPEC.md
```

实现提示（V1）：
- `server/app/core` 中定义所有共享 schema（ExecutionPlan、EvidencePack、BJJ Answer JSON、Trace 结构）；`web/lib` 通过生成或复制方式对齐（避免前后端字段漂移）。
- `data/` 默认加入 `.gitignore`；评测与 SFT 数据放 `datasets/` 才能回归对比。
...
V1 推荐 **RRF（Reciprocal Rank Fusion）** 作为第一阶段融合：
...
#### 5.3.1 Optional Cross-Encoder Reranker（V1 可选二阶段重排）
当 `runtime_config.reranker.enabled=true` 时，V1 允许在 RRF 候选集之后接一个 **cross-encoder reranker**：
- 位置：`structured + BM25 + dense -> RRF -> candidate pool -> cross-encoder rerank -> diversity/token trim -> Evidence Pack`
- 目的：在不替换第一阶段 hybrid retrieval 的前提下，对前若干候选做更精细的 query-document 匹配重排。
- 候选范围：只对 RRF 头部候选做重排；候选数由运行时配置控制（例如 `candidate_pool_multiplier` / `max_candidates`）。

V1 行为约束：
- `fake` profile：使用确定性 mock reranker，保证测试、smoke、replay 不依赖真实模型。
- `real` profile：允许使用 OpenAI-compatible provider 执行一次性 rerank 打分。
- provider 不可用、超时、返回 JSON 非法或 schema 非法时：**必须回退到原 RRF 顺序**，不能让 retrieval 失败。
- reranker 不是新的检索入口；它只能重排既有候选，不能引入候选集之外的新 chunk。
...
- `EvidencePack.rank_signals` 必须允许记录：
  - `cross_encoder_rank`
  - `cross_encoder_score`
- Frozen replay 默认仍消费冻结后的 Evidence Pack，不要求重新执行 reranker；rerank telemetry 只作为审计与评估输入。
...
### 8.2 检索策略（Dense/Sparse + RRF + optional rerank，取 top-3 文档）
输入：
...
- Fusion：RRF
- Optional rerank：对 RRF 头部候选做 cross-encoder 重排，然后再按 `doc_id` 去重取 top-3 文档
...
> 目标：把第 6 章 Orchestrator（澄清/Probe/Replan）、第 7 章 Coach Gate（三态输出）、第 10 章 Tracing、以及第 13 章 SFT 需要的“可回放/可导出/可核验”能力落到接口层。
...
输入：
- `model_variant`：`base` | `policy`（V1 必须支持 base/policy 对比；policy 由第 13 章训练产生）
...
> 原则：把系统做成“可回放、可归因、可训练（含 SFT）”的工程。Trace 既服务 debug，也服务 Golden set 回放与 SFT 数据闭环。
...
## 13. Policy SFT（V1 必做：行为策略微调）

> V1 要求：跑通一条最小可用闭环——trace 导出数据集 → 人工修订少量样本 → 训练一个 LoRA/QLoRA policy checkpoint → 接入 `model_variant=policy` 的 replay 对比评测。线上默认可以仍用 base，但必须能一键切换并在 Evaluation 中展示提升与退化。
...
### Phase 1（V1 必交付）— “闭环可回放 + SFT MVP”
- Ingestion：BJJ 强制格式解析 + NOTES chunking；safe_summary job（可重试/可观测）
- Retrieval：structured filter + FTS5(BM25) + Chroma(dense) + RRF + optional cross-encoder rerank；Evidence Pack 冻结与引用回放
- Orchestrator：PROBE + plan_check +（必要时）LLM replan + Clarify（≤2）
- Agents：
  - BJJ Coach：Gate 三态 +（AMBIGUOUS 追问 opponent_control ≤1）+ 三态 JSON 输出 + Validator + repair/degrade
  - Literary：top-1 raw_excerpt + top-2/3 safe_summary anchors + 创作性生成
- Observability：trace/span/event 全链路打点；runtime_config_snapshot；成本/延迟统计；Frozen Evidence Replay
- Evaluation：Golden set v0（≥30 条）+ 硬指标回归 + RAGAS（趋势）+ LLM-as-judge（抽样）+ 人工 rubric（抽检）
- SFT（必须跑通一条闭环）：
  - trace 导出 `datasets/sft/v1/...`（manifest+jsonl）
  - 人工修订少量高价值样本（建议先 50–100 条起步，再扩到 200–500）
  - 训练 1 个 LoRA/QLoRA policy checkpoint
  - 接入 `model_variant=policy`，并在 replay/eval 中展示 base vs policy 指标对比

### Phase 2（V1.1）— “体验与运维强化”
- UI：Retrieval Logs/Trace Viewer 完善（失败标签、validator 错误可视化）
- 运维：reindex/reembed 一键任务、job 失败重试与告警、embedding_version 切换策略固化
- Orchestrator：时间解析覆盖扩展（中文相对时间/跨年）与阈值调参工具化（网格搜索）

### Phase 3（V1.2）— “质量与成本优化”
- 缓存：embedding/query rewrite 缓存、重复 evidence_pack 去重
- 质量：Gate 阈值与 reason_codes 基于 golden set 迭代；SFT 数据集扩充与回归
- 性能：端到端延迟分解优化（按 span 定位瓶颈）
...
4) **反对用 SFT 替代 Gate/Validator**：V1 即便接入 policy 模型，也必须保留规则 Gate、严格 schema、validator 与 repair/degrade；否则一旦模型退化，你的系统将失去可审计性与可回归性。

    ...
    def test_create_app_state_loads_local_env_file(self) -> None:
        ...
        try:
            ...
            with TemporaryDirectory() as tmp:
                ...
                self.assertFalse(retrieval_status["configured"])
                self.assertEqual(retrieval_status["model"], "BAAI/bge-reranker-base")
                self.assertIsNone(retrieval_status["base_url"])
                self.assertIn("torch", retrieval_status["missing_dependencies"])
                self.assertIn("transformers", retrieval_status["missing_dependencies"])
                ...
                self.assertFalse(eval_status["ragas"]["configured"])
                self.assertEqual(eval_status["ragas"]["base_url"], "https://example.invalid/v1")
                self.assertIn("ragas", eval_status["ragas"]["missing_dependencies"])
